...
void Tweets_Load(string filename)
{
	/*---------------------------------------------------------------------------*/
	//Start dealing with data file
	string dist = "../Data/";
	dist.append(filename);
	cout << "\nReading file: " << dist << "\n";
	ifstream file(dist); // pay attention to the address of PROJECT
	string tweet_line;
	int count_line = 0;
	bool flag_handle = TRUE; // DBInit will delete its handles
	...
	while (flag_while)
	{
		...
		// Condition check for whether start SQL command or not
...
VALUES ( " << s_tweetID << ", '" << s_tweetTime << "', " << s_reply_to_ID << " )\nEND\n";
			...
			SQL_DB_3Comd(Comd1.str(), Comd2.str(), Comd3.str());
...
END\n";
				// convert stringstream to string, and load into function
				SQL_DB_3Comd(Comd31.str(), Comd32.str(), Comd33.str());
				++TagIDit;
				++Tagit;
...
// ASC char -> corresponding int on chart -> devide by 64 (thus each letter with 2 digits) -> stringstream together
...
//for tweetID_MenUserList and tweetID_TagList
//INSERT INTO Data ( Col1 ) VALUES
//('Hello'), ('World'), ...
//use for list loop and stringstream to generate ('Hello'), ('World'), ...

// each tweet, at least 3 comds; 1 comd if has MenUser; 3 comds if has tag

//IF Just Reply_to 
// comd0 for tweet_Stack, userID_Unique, userID_Call

//IF has MenUser
// comd1 for tweetID_MenUserList

//IF has tag
// comd1 for Tag_Unique, comd2 for TagID_Call, comd3 for tweetID_TagList