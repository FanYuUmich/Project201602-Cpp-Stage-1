...
//int SQL_DB_Init(SQLHANDLE& sqlenvhandle, SQLHANDLE& sqlconnectionhandle, SQLHANDLE& sqlstatementhandle, SQLRETURN& retcode)
int SQL_DB_Init() {
	...
	if (flag_Connection = TRUE)
	{
		switch (SQLDriverConnect(sqlconnectionhandle,
			...
			// "DRIVER={SQL Server};SERVER={FANYU-HC\\SQLEXPRESS};DATABASE={SQLTutorial};Trusted_connection={yes};user_id={sa};password={fanyu01};
			/*---------------------------------------------------------------------*/
			SQL_NTS,
			retconstring,
			1024,
	...
	/*---------------------------------------------------------------------*/
	// SQL Commands
	SQLCHAR* SQL_Query = (SQLCHAR*)"  USE DBTestv1 "; // Doesn't seem necessary
